...
export class ThermostatAccessory {
  ...
  constructor(
    ...
    this.charParams = {
      ...
      TargetHeatingCoolingState: {required: false, get: true, set: true},
      CurrentTemperature: {required: false, set: true, get: true},
    ...
    // get the LightBulb service if it exists, otherwise create a new Thermostat service
    ...
    for (const char in this.charParams) {
      ...
      if (accessory.context.device.characteristics[char] !== undefined) {
        ...
        if (this.platform.config.remoteApiCharPoll[0].Thermostat.enabled && this.platform.config.remoteApiCharPoll[0].Thermostat[char]) {
          setInterval (async () => {
